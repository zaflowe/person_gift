...
import { ProjectLongTaskTemplate } from "@/lib/api/project-long-tasks";
import { cn } from "@/lib/utils";

interface EditLongTaskModalProps {
    template: ProjectLongTaskTemplate;
    onClose: () => void;
    onSave: (templateId: string, payload: any) => Promise<void>;
}

const WEEKDAYS = ["一", "二", "三", "四", "五", "六", "日"]; // 0=Mon
const PRESET_CYCLES = [7, 28, 365];

export function EditLongTaskModal({ template, onClose, onSave }: EditLongTaskModalProps) {
    const [loading, setLoading] = useState(false);
    const [form, setForm] = useState({
        title: template.title || "",
        frequency_mode: template.frequency_mode || "interval",
        interval_days: template.interval_days || 1,
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            <div className="bg-white border border-slate-200 rounded-xl shadow-xl w-full max-w-lg flex flex-col max-h-[90vh] overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                    <h2 className="text-lg font-bold text-slate-800">编辑长期任务</h2>
