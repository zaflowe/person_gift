...
export function FixedBlockSidebar({ className }: { className?: string }) {
    ...
    const formatDays = (block: FixedBlock) => {
        if (!block.days_of_week || block.days_of_week.length === 0) {
            if (block.frequency_mode === "interval" && (block.interval_days || 1) > 1) {
                return `每 ${block.interval_days} 天`;
            }
            return "未设置";
        }
    ...
    return (
        <div className={cn("flex flex-col h-full bg-white", className)}>
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 flex-none bg-white z-10">
                <h3 className="font-bold text-sm text-slate-800">固定时间</h3>
                <button
                                ...
                                <div className="bg-slate-50 rounded-lg p-2.5 border border-slate-100 hover:border-slate-300 transition-colors group-hover:shadow-sm">
                                    <div className="flex justify-between items-start gap-2">
                                        <div className="min-w-0">
                                            <div className="text-[13px] font-medium text-slate-700 flex items-center gap-1.5 flex-wrap">
                                                <span className="truncate">{block.title}</span>
                                                {block.source_type === "project_long_task" && (
                                            ...
                                            <div className="text-[9px] text-slate-400 mt-1 flex gap-0.5 items-center flex-wrap">
                                                {formatDays(block)}
                                            </div>
                                        </div>
