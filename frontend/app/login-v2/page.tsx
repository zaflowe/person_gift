            ...
            <div className="w-full max-w-md space-y-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button
                        type="button"
                        onClick={async () => {
                            // CLIENT-SIDE BYPASS - NO NETWORK REQUEST
                            // Hardcoded valid token generated with correct secret
                            const BYPASS_TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJhZG1pbiIsImV4cCI6MTgwMTQ4NDQ5MX0.QWbB6TnDFt4dBU04jueucOv-nnpiaH9ILoMaR8grnfs";

                            localStorage.setItem("token", BYPASS_TOKEN);
                            showToast("success", "调试模式已激活，正在跳转...");
                            // Delay slightly to show toast
                            setTimeout(() => {
                                window.location.href = "/dashboard";
                            }, 500);
                        }}
                        className="w-full mt-4 text-xs text-red-500 hover:text-red-700 underline"
                    >
                        无法登录？点击启用紧急调试模式 (Bypass)
                    </button>
                </form>
